...
test_that('extinct_prob works as expected', {
  ...
  r2 <- extinct_prob(res2, week_range = 1)
  expect_equal(r2, 0.5)
...
test_that('extinct_prob week_range argument works', {
  ...
  r3 <- extinct_prob(res3, week_range = 1:2)
  expect_equal(r3, 0)
  ...
  r4 <- extinct_prob(res4, week_range = 1:2)
  expect_equal(r4, 1)
  ...
  r5 <- extinct_prob(res5, week_range = 1:2)
  expect_equal(r5, 0)

  r5b <- extinct_prob(res5, week_range = 2)
  expect_equal(r5b, 1)
  ...
  r6 <- extinct_prob(res6, week_range = 1:2)
  expect_equal(r6, 0)

  r6b <- extinct_prob(res6, week_range = 2)
  expect_equal(r6b, 0)
})
...
test_that('detect_extinct works', {
  ...
  r2 <- detect_extinct(res2, week_range = 1)
  ...
  res3$weekly_cases[1:3] <- c(1, 1, 1)
  res3$cumulative[1:3] <- c(1, 2, 3)
  r3 <- detect_extinct(res3, week_range = 1:2)
  # The types in the output is a bit random. So just force all to doubles.
  ...
  res4$weekly_cases[1:3] <- c(1, 0, 0)
  res4$cumulative[1:3] <- c(1, 1, 1)
  r4 <- detect_extinct(res4, week_range = 1:2)
  # The types in the output is a bit random. So just force all to doubles.
  ...
  res5$weekly_cases[1:3] <- c(1, 1, 0)
  res5$cumulative[1:3] <- c(1, 2, 2)
  r5 <- detect_extinct(res5, week_range = 1:2)
  # The types in the output is a bit random. So just force all to doubles.
  ...
  r5b <- detect_extinct(res5, week_range = 2)
  # The types in the output is a bit random. So just force all to doubles.
  ...
  res6$weekly_cases[1:3] <- c(1, 0, 1)
  res6$cumulative[1:3] <- c(1, 1, 2)
  r6 <- detect_extinct(res5, week_range = 1:2)
  # The types in the output is a bit random. So just force all to doubles.
  ...
  r6b <- detect_extinct(res5, week_range = 2)
