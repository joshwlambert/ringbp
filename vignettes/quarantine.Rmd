...
From this outbreak we can look at the effecive reproduction number ($R_t$) from each of the `r nsim` simulation replicates, as well as the number of generations (in terms of branching process generations) for each simulation replicate ($N_\mathrm{gen}$).
...
cols_to_keep <- setdiff(names(pars), c("Rt", "n_gen"))
compare_Q_dists <- pars[, .(Rt = unlist(Rt)), by = cols_to_keep]
...
tinyplot(
  ~ Rt | quarantine, 
  data = compare_Q_dists[asymptomatic == 0 & symptomatic_ascertained == 1, ], 
  type = "density", 
  fill = "by",
  facet = ~delay_type,
  theme = "clean"
...
compare_Q_summary <- compare_Q_dists[
  ...
  by = .(asymptomatic, symptomatic_ascertained, quarantine, delay_type)
]
```
...
compare_Q_summary_w <- dcast(
  compare_Q_summary, 
  asymptomatic + symptomatic_ascertained + delay_type ~ quarantine, 
...
# format column names
col_names <- c(
  "% Asym.", "% CT", "O2I Delay", "Mean $\\\\Delta R_t$",
  "Median $\\\\Delta R_t$", "Mean $\\\\Delta N_\\mathrm{gen}$",
