...
For this example we will run 5 replicates for each parameter set. The simulation model is stochastic so by running multiple replicates for each, we can understand the variance within scenarios. For a scientifically robust analysis, the number of replicates should be much higher (e.g. 100-1000).
...
In this example we run the same scenario parameter sweep as before (again with 5 replicates), but store the simulated outbreak `data.table`s in the same object as the list of scenarios. We append the simulation results to the `scenario_sims` `data.table` and assign them to the `sims` column.
...
Here we use run the analysis on 2 separate R sessions running in the background.
